<script>
  ...
  import { loadPokemonByMove } from '../stores/pokemonStore.js';
  ...
  async function handleViewPokemon() {
    ...
    try {
      console.log('🔍 Cargando Pokemon para el movimiento:', selectedMoveData.name);
      await loadPokemonByMove(selectedMoveData.name);
      console.log('✅ Pokemon cargados exitosamente');
      
      // Aquí podrías navegar a la página de lista de Pokemon
      // router.push('/pokemon/' + selectedMoveData.name);
      
    } catch (err) {
      console.error('❌ Error al cargar Pokemon:', err);
      pokemonError = err.message || 'Error al cargar los Pokemon';
    } finally {
